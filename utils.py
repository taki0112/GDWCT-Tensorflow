...
class ImageData:
    ...
    def image_processing(self, filename):
        ...
        if self.augment_flag :
            augment_size_h = self.img_h + (30 if self.img_h == 256 else int(self.img_h * 0.1))
            augment_size_w = self.img_w + (30 if self.img_w == 256 else int(self.img_w * 0.1))

            # p = random.random()
            # if p > 0.5:
            #     img = augmentation(img, augment_size_h, augment_size_w)

            img = tf.cond(pred=tf.greater(tf.random_uniform(shape=[], minval=0.0, maxval=1.0), 0.5),
                          true_fn=lambda: augmentation(img, augment_size_h, augment_size_w),
                          false_fn=lambda: img)
